...
class CarSale(models.Model):
    ...
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name="list_sales", null=True)
    ...
    def set_final_price(self):
        self.buying_amount = self.car.price
        if self.discount:
            self.discount_amount = self.buying_amount * (self.discount.percent / 100)
            self.final_sum = self.buying_amount - self.discount_amount
        else:
            self.discount_amount = 0
            self.final_sum = self.buying_amount
