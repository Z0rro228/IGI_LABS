...
def statistics(request: HttpRequest):
    ...
    orders = CarSale.objects.all().order_by("car").order_by("car")
    
    total_sum = CarSale.objects.aggregate(total_sum=Sum('final_sum'))["total_sum"]
    
    sales = CarSale.objects.values_list("final_sum", flat=True)
    ...
    profitable_brand = CarBrand.objects.annotate(total_sales=Sum('car__carsale__final_sum')).order_by('-total_sales').first()
...
def list_users(request: HttpRequest):
    ...
    lst = []
    for client in clients:
        orders = CarSale.objects.filter(client=client, is_active=True)
        sum = 0
        for order in orders:
            sum += order.final_sum
        lst.append(sum)
...
def delete_user(request: HttpRequest, user_id):
    client = Client.objects.get(pk=user_id)
    orders: list[CarSale] = CarSale.objects.filter(client=client)
    for order in orders:
        car = order.car 
